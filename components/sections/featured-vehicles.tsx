...
async function FeaturedVehicleData() {
  ...
  return <section className="featured-vehicles container" data-reveal><div className="section-heading"><div><p className="eyebrow muted">GESTIONES PUBLICADAS</p><h2 className="section-title">Vehículos destacados.</h2></div><a className="quiet-link" href="/vehiculos">Ver todos <ArrowRight /></a></div><div className="vehicle-grid">{vehicles.map((vehicle) => <VehicleCard key={vehicle.id} vehicle={vehicle} />)}</div></section>;
}
